...
### PAC vs 广告屏蔽规则对比（小白速览）

| 项目          | 主要干嘛？                  | 最常用在哪？                  | 能挡什么？                     | 路径例子                              | 小白比喻          |
|---------------|-----------------------------|-------------------------------|--------------------------------|---------------------------------------|-------------------|
| **PAC 文件** | 自动决定走代理还是直连      | SwitchyOmega、Clash PAC、Windows 系统代理 | 域名/IP（墙外代理、墙内直连）<br>老项目顺带挡广告域名（不准） | 本地：`http://127.0.0.1:21883/pac?...`<br>远程：`http://pac.机场.com/pac` | 交通警察：管“走哪条路” |
| **广告屏蔽规则** | 专门杀广告、弹窗、追踪器   | uBlock Origin、AdGuard       | 域名 + 路径 + 元素隐藏（超准） | 订阅链接：`https://anti-ad.net/...`<br>插件内加规则（不填系统路径） | 屋里清洁工：扫垃圾 |
